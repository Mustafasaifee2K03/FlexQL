...
static const int DEFAULT_TOTAL_COLUMNS = 100;
...
static const size_t MAX_BULK_REQUEST_BYTES = 8 * 1024 * 1024;
...
static bool run_insert_benchmark(FlexQL *db,
    ...
    if (!run_exec(db, create_sql, "CREATE TABLE BIG_USERS (" + to_string(total_columns) + " COLS)")) {
        return false;
    }

    cout << "\nStarting insertion benchmark for " << target_rows << " rows (" << total_columns
            << " columns, batch=" << batch_size << ", client_threads=" << client_threads << ")...\n";
    auto bench_start = high_resolution_clock::now();
    ...
    // Pre-build dynamic extra-column values as integers.
    ...
    for (int worker = 0; worker < workers; ++worker) {
        ...
        threads.emplace_back([&, worker, rows_for_worker, start_index]() {
            (void)worker;
            ...
            auto flush_bulk = [&](size_t *bulk_bytes) -> bool {
                ...
                char *errMsg = nullptr;
                const int rc = flexql_exec_bulk(worker_db, sql_ptrs.data(), static_cast<int>(sql_ptrs.size()), &errMsg);
                if (rc != FLEXQL_OK) {
            ...
            while (local_done < rows_for_worker && !failed.load(memory_order_acquire)) {
                ...
                const string id_s = to_string(id);

                string row_sql;
                row_sql.reserve(300 + static_extra_cols.size());
                ...
                row_sql.append("', 'user");
                row_sql.append(id_s);
                row_sql.append("@mail.com', ");
                row_sql.append(to_string(1000 + (id % 10000)));
                row_sql.append(", 1893456000");
                row_sql.append(static_extra_cols);
                row_sql.append(") EXPIRES 1893456000;");
...
int main(int argc, char **argv) {
    ...
    auto parse_i64 = [](const char *s, long long *out) -> bool {
        if (!s || !out) {
            return false;
        }
        try {
            size_t parsed = 0;
            long long v = stoll(string(s), &parsed, 10);
            if (parsed != string(s).size()) {
                return false;
            }
            *out = v;
            return true;
    ...
    if (argc > 1 && string(argv[1]) == "--unit-test") {
        run_unit_tests_only = true;
    } else {
        if (argc > 1) {
            if (!parse_i64(argv[1], &insert_rows) || insert_rows <= 0) {
                cout << "Invalid target_rows. Expected a positive integer.\n";
                return 1;
            }
        }

        long long parsed_columns = total_columns;
        if (argc > 2) {
            if (!parse_i64(argv[2], &parsed_columns) || parsed_columns < 5) {
                cout << "Invalid total_columns. Expected an integer >= 5.\n";
                return 1;
            }
        }
        total_columns = static_cast<int>(parsed_columns);

        long long parsed_batch = batch_size;
        if (argc > 3) {
            if (!parse_i64(argv[3], &parsed_batch) || parsed_batch <= 0 || parsed_batch > MAX_BATCH_SIZE) {
                cout << "Invalid batch_size. Expected 1.." << MAX_BATCH_SIZE << ".\n";
                return 1;
            }
        }
        batch_size = static_cast<int>(parsed_batch);

        long long parsed_threads = client_threads;
        if (argc > 4) {
            if (!parse_i64(argv[4], &parsed_threads) || parsed_threads <= 0 || parsed_threads > 256) {
                cout << "Invalid client_threads. Expected 1..256.\n";
                return 1;
            }
        }
        client_threads = static_cast<int>(parsed_threads);

        if (argc > 5) {
            cout << "Usage: " << argv[0] << " [target_rows] [total_columns>=5] [batch_size<="
                 << MAX_BATCH_SIZE << "]\n";
            cout << "      [client_threads<=256]\n";
